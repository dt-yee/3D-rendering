...
*/
	...
	for (int i = 0; i < numParts; i++) {
		if (nameList[i] == GZ_POSITION) {
			...
			//X is final order
			// Ymin,Ymid,Ymax clockwise 1->2->3
			int X1 = Ymin, X2 = Ymid, X3 = Ymax;
			
			////if two Ymin, a parallel
			if (coord[Ymin][Y] == coord[Ymid][Y]) {
				//Ymid.x > Ymin.x, Ymin->Ymid->Ymax
				if (coord[Ymin][X] < coord[Ymid][X]) {					
					X1 = Ymin;
					X2 = Ymid;
					X3 = Ymax;
				}
				...
				// no parallel, Ymin-> maxX(Ymid, Ymax) -> minX(Ymid,Ymax)
				X1 = Ymin;
				if (coord[Ymid][X] < coord[Ymax][X]) {
					X2 = Ymax;
					X3 = Ymid;
				}
				else {
